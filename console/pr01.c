...
fillMemoryWithRandomValues (int count)
{
  ...
  for (int i = 0; i < count && i < MEMORY_MAX_SIZE; i++)
    {
      int randomValue
          = rand () % 65536; // Генерация случайного числа от 0 до 65535
      if (rand () % 2 == 0)
        { // Случайно делаем число отрицательным
...
main ()
{
  // a. Инициализация оперативной памяти, аккумулятора, счетчика команд и
  // регистра флагов
  sc_memoryInit ();
  sc_accumulatorInit ();
  sc_icounterInit ();
  ...
  // Вывод содержимого памяти (в декодированном формате по 10 ячеек в строке
  // через пробел)
  ...
  // d. Установка произвольных значений флагов и вывод содержимого регистра
  // флагов
  ...
  // j. Декодирование значения произвольной ячейки памяти и значение
  // аккумулятора
  ...
  // k. Кодирование команды и вывод полученного значения в разных системах
  // счисления
  int encodedValue;
